...
const userDataValidation = ({email,username,password})=>{
    return new Promise((resolve,reject)=>{
        ...
        if(!username) reject("Username field cannot be empty");
        if(!password) reject("Password field cannot be empty");
...
const loginDataValidation = ({loginId, password})=>{
    return new Promise((resolve,reject)=>{
        if(!loginId) reject("LoginId field cannot be empty");
        if(!password) reject("Password field cannot be empty");
        if(typeof loginId !== 'string') reject ("loginId is not a text");
        if(typeof password !== 'string') reject ("Password is not a text");
         resolve(); 
    })
}
...
const sendEmailVerificationMail = (email,token)=>{
    const transporter = nodemailer.createTransport({
        ...
        service: "gmail",
        auth:{
            user: "raghavkrtonk@gmail.com",
            pass: "lknz cebb tvns uvwh"
        }
    });
    const mailOptions = {
        from: 'raghavkrtonk@gmail.com',
        to: email,
      ...
      <td bgcolor="#ffffff" style="padding: 40px 30px 40px 30px;">
        <table border="0" cellpadding="0" cellspacing="0" width="100%" style="border-collapse: collapse;">
          ...
          <tr>
            ...
            <a href="https://blog-app-backend-huph.onrender.com/auth/verify/${token}" style=" border: none;
    ...
    <tr>
      ...
      transporter.sendMail(mailOptions, function(error, info){
        if (error) {
          ...
          console.log(`Email has been send successfully:${email} ` + info.response);
        }
    });
}
module.exports = {userDataValidation,loginDataValidation, genrateToken,sendEmailVerificationMail};